...
public class MineWorkGridAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public MineWorkGridAdapter(Context context, boolean isCustom) {
        super();
        this.mContext = context;
        this.isCustom = isCustom;
    }

    public MineWorkGridAdapter(Context context, ArrayList<WorkItem> mList) {
        super();
        this.mContext = context;
        this.mList = mList;
    ...
    public MineWorkGridAdapter(Context context, ArrayList<WorkItem> mList, boolean isCustom) {
        super();
        this.mContext = context;
        this.mList = mList;
        this.isCustom = isCustom;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (isCustom) {
            if (viewType == ADD_TYPE) {
                ...
                return new WorkListViewHolder(LayoutInflater.from(mContext).inflate(R.layout.work_list_item, parent, false));
            }
        } else {
            return new WorkListViewHolder(LayoutInflater.from(mContext).inflate(R.layout.work_list_item, parent, false));
        }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        if (getItemViewType(position) == ADD_TYPE) {
            ...
            workListViewHolder.content.setText(item.getContent());
            workListViewHolder.like.setText("" + item.getLike_count());
            workListViewHolder.comment.setText("" + item.getComment_count());
...
//                Netroid.displayImage(Util.transfer(item.getImages().get(0).getUrl()), workListViewHolder.image);
