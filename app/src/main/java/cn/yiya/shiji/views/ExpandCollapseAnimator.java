...
public class ExpandCollapseAnimator implements ValueAnimator.AnimatorUpdateListener, Animator.AnimatorListener {
    ...
    public void onAnimationUpdate(ValueAnimator animation) {
        ...
        final float d = speed * getInterpolation((float) dt);
    ...
    private boolean expandingDone() {
        return (compare(positionToViewMap.get(expandingViewPosition).getExpandCoef(), 1, epsilon) == 0 &&
                (!positionToViewMap.containsKey(expandingViewPosition + 1) || compare(positionToViewMap.get(expandingViewPosition + 1).getExpandCoef(), 0, epsilon) == 0));
    }
