 ...
 */
public class TravelMallActivity extends AppCompatActivity implements AppBarLayout.OnOffsetChangedListener, View.OnClickListener {
    ...
    private LinearLayout llytMallName;                              // 显示商场名字的布局
    private TextView tvMallName;                                    // 商场英文名
    private TextView tvMallCnName;                                  // 商场中文名
    ...
    private ArrayList<StoreCategoryInfo> mTitle = new ArrayList<>();  // 热门店铺tab标题
    String name;
    String country;
    String address;
...
//                collapsingToolbar.setExpandedTitleColor(titleColor);
        ...
        llytMallName = (LinearLayout) findViewById(R.id.llyt_mall_name);
        tvMallName = (TextView) findViewById(R.id.tv_name);
        tvMallCnName = (TextView) findViewById(R.id.tv_cn_name);
    ...
    private void getMallInfo(String id) {
        if (bNet) {
            new RetrofitRequest<MallDetailInfo>(ApiRequest.getApiShiji().getMallDetailInfo(id)).handRequest(
                    new MsgCallBack() {
                        @Override
            ...
            GetLocalRequest.getInstance().getLocalMallInfo(mHander, id, countryId, cityId, new MsgCallBack() {
                @Override
                public void onResult(HttpMessage msg) {
                    handleMallMsg(msg);
                }
    ...
    private void handleMallMsg(HttpMessage msg) {
        totalHttpCount++;
        if (msg.isSuccess()) {
            bLoadInfo = true;
            ...
            tvMallName.setText(info.getName());
            tvMallCnName.setText(info.getCn_name());

        } else {
            if (!NetUtil.IsInNetwork(TravelMallActivity.this)) {
                bLoadInfo = true;
                mInfos = null;
            }
    ...
    private void  getCoupon(String id) {
        ...
        if (bNet) {
            ...
            new RetrofitRequest<CouponDetailObject>(ApiRequest.getApiShiji().getCouponListOfMall(maps)).handRequest(
                    new MsgCallBack() {
                        @Override
                        public void onResult(HttpMessage msg) {
                            handleCouponMsg(msg);
            ...
            GetLocalRequest.getInstance().getLocalCouponList("mall", id, cityId, mHander, countryId, new MsgCallBack() {
                @Override
                public void onResult(HttpMessage msg) {
                    handleCouponMsg(msg);
                }
        ...
        new RetrofitRequest<StoreCategoryObject>(ApiRequest.getApiShiji().getStoreBrandsCategoryOfMall(maps))
            ...
            public void onResult(HttpMessage msg) {
                ...
                if (msg.isSuccess()) {
                    ...
                    mTitle = info.getCategory_list();
                    mTitle.add(0, storeCategoryInfo);
                    ...
                    if (!NetUtil.IsInNetwork(TravelMallActivity.this)) {
                        ...
                        storeCategoryInfo.setId("");
                        storeCategoryInfo.setName("全部");
                        mTitleLists.add(storeCategoryInfo);
                        setTotalAdapter();
                    }
                }
            }
    ...
    private void getRecommend(String id) {
        ...
        new RetrofitRequest<RecommendList>(ApiRequest.getApiShiji().getMallRecommendInfo(id)).handRequest(
                new MsgCallBack() {
                    @Override
                    public void onResult(HttpMessage msg) {
                        totalHttpCount++;
    ...
    public void onOffsetChanged(AppBarLayout appBarLayout, int i) {
        ...
        llytMallName.setAlpha((1 - percentage));
...
//                imageUrl, url, mInfos.getShare_info().getDes());
    ...
    private synchronized void setTotalAdapter() {
        ...
        if (bLoadInfo && bLoadBanner && bLoadRecommend && bLoadTitle && bLoadStore && mInfos != null && mInfos.getName() != null) {
            mAdapter = new NearbyShopAdapter(this, mInfos, mBannerLists, mTitleLists,
                    mStoreLists, mRecommendLists, countryId, cityId, id, bNet, 2);
            ...
            if (mStoreLists.size() > 2) {
                moreShop = new JoinableLayout(R.layout.bottom_more_data_layout, new JoinableLayout.Callback() {
                    ...
                    public void onInflateComplete(View view, ViewGroup parent) {
                        ...
                        llytShopMore.setOnClickListener(TravelMallActivity.this);
                    }
                });
    ...
    private void beNullData() {
        if (!bLoadInfo && !bLoadBanner) {
            ...
            hidePreDialog();
            return;
        } else if (bLoadInfo || bLoadBanner || bLoadRecommend || bLoadTitle || bLoadStore) {
        ...
        boolean bHotShop = mTitleLists == null || mTitleLists.size() == 0 ? true : false;
        ...
        if(bBanner && bRecommend && bNearStore && bHotShop && bMall){
            appBarLayout.setVisibility(View.GONE);
            rycvNearbyShop.setVisibility(View.GONE);
            relativeLayout.setVisibility(View.VISIBLE);
            rlTitle.setVisibility(View.VISIBLE);
