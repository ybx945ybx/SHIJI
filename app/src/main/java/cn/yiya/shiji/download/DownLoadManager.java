...
public class DownLoadManager {
    ...
    /**用户ID,默认值man*/
    private String userID = BaseApplication.FileId;
...
//        userID = sharedPreferences.getString("UserID", BaseApplication.userInfo.getUser_id() + "");
     ...
     */
    public int addTask(String countryId, String cityId, String TaskID, String url, String fileName) {
        return addTask(countryId, cityId, TaskID, url, fileName, null);
    }
     ...
     * @return -1 : 文件已存在 ，0 ： 已存在任务列表 ， 1 ： 添加进任务列表
     */
    public int addTask(String countryId, String cityId, String TaskID, String url, String fileName, String filepath) {
        if(TaskID == null){
            TaskID = fileName;
        }
        int state = getAttachmentState(countryId, cityId, TaskID, fileName, filepath);
        if (state != 1) {
            return state;
        }
        ...
        downloadinfo.setCountryID(countryId);
        downloadinfo.setCityID(cityId);
        if (filepath == null) {
            downloadinfo.setFilePath(FileHelper.getFileDefaultPath() + "/" + countryId + "/" + cityId + "/" + fileName);
        } else {
        ...
        taskDownLoader.setSupportBreakpoint(isSupportBreakpoint);
     ...
     * @return  -1 : 文件已存在 ，0 ： 已存在任务列表 ， 1 ： 添加进任务列表
     */
     private int getAttachmentState(String countryId, String cityId, String TaskID, String fileName, String filepath) {
        ...
        File file = null;
        if (filepath == null) {
            file = new File(FileHelper.getFileDefaultPath() + "/"  + countryId + "/" + cityId + "/" + fileName);
            if (file.exists()) {
                return -1;
            }
        } else {
    ...
    public ArrayList<TaskInfo> getAllTask() {
        ...
        for (int i = 0; i < listSize; i++) {
            ...
            boolean bDownload = sqldownloadinfo.getDownload().equals("T") ? true : false;
            if (bDownload) {
                deletedownloader.start();
            }
            taskinfo.setOnDownloading(bDownload);
            ...
            if (sqldownloadinfo.getDownloadSize() >= sqldownloadinfo.getFileSize()
                    && sqldownloadinfo.getDownloadSize() > 0) {
                String unZipStatus = sqldownloadinfo.getbUnZip();
                if (!TextUtils.isEmpty(unZipStatus) && unZipStatus.equals("T")) {
                    taskinfo.setHasDownload(true);
                }
            }
    ...
    public void saveAllTask() {
        ...
        for (int i = 0; i < listSize; i++) {
            DownLoader deletedownloader = taskList.get(i);
            SQLDownLoadInfo sqldownloadinfo = deletedownloader.getSQLDownLoadInfo();
            deletedownloader.saveDownLoadInfoStatus(sqldownloadinfo.getDownload());
            deletedownloader.saveDownLoadInfoZip(sqldownloadinfo.getbUnZip());
        }
    }
